...
try {
    $filename = \Ease\Functions::cfg('STATEMENTS_DIR', sys_get_temp_dir()) . '/' . strtolower(\Ease\Shared::cfg('FIO_TOKEN_NAME')) . '-' . $start->format('Y') . '_' . $start->format('n') . '.' . $format;
    $statements[$start->format('n')] = [\Ease\Shared::cfg('FIO_TOKEN_NAME') => \Ease\Shared::cfg('ACCOUNT_NUMBER')];
